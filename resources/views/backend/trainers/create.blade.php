...
@section('title', 'Admin | Add Trainer')
@section('content')
...
<div id="page-wrapper">
    <div class="container-fluid">
            ...
            @if($permissiondata->contains('trainers.create') && $permissiondata->contains('trainers.index') || $userdata->id == 1)
            <div class="row">
                <div class="col-lg-12 col-md-12">
                    <div class="panel panel-default">
                        <div class="panel-body">
                            <div class="row">
                                <form role="form" style="padding:1rem 2rem;" method="post" action="{{ route('admin.dashboard.trainer.store') }}" enctype="multipart/form-data">
                                    @csrf
                                    <div class="form-group">
                                        <label>Trainer Name</label>
                                        <input class="form-control @error('trainer_name') is-invalid @enderror" id="trainer_name" type="text" name="trainer_name" value="{{ old('trainer_name') }}">
                                        @error('trainer_name')
                                    ...
                                    <div class="form-group banner-image-field">
                                        <label>Email</label>
                                        <input class="form-control @error('email') is-invalid @enderror" id="price" name="email" value="{{ old('email') }}">
                                        @error('email')
                                            <div class="formAlertError alert alert-danger">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control  @error('bio') is-invalid @enderror" id="description" name="bio">{{ old('bio') }}</textarea>
                                    </div>


                                    <div class="form-group banner-image-field">
                                        <label>Password</label>
                                        <input @error('password') is-invalid @enderror" type="password" class="form-control id="password" name="password" value="{{ old('password') }}">
                                        @error('password')
                                            <div class="formAlertError alert alert-danger">{{ $message }}</div>
                                        @enderror
                                    </div>
