...
<div id="page-wrapper">
    <div class="container-fluid">
            <div class="row">
                <div class="col-lg-12">
                    <div class="row">
                        <h1 class="page-header">Edit {{ $admintrainer->trainer_name }} <span></h1>
                    </div>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            ...
            @if($permissiondata->contains('trainers.edit') || $userdata->id == 1)
            <div class="row">
                <div class="col-lg-12 col-md-12">
                    <div class="panel panel-default">
                        <div class="panel-body">
                            <div class="row">
                                <form role="form" style="padding:1rem 2rem;" method="post" action="{{ route('admin.dashboard.trainer.update', $admintrainer->id) }}" enctype="multipart/form-data">
                                    @csrf
                                    @method('PUT')
                                    <div class="form-group">
                                        <label>Trainer Name</label>
                                        <input class="form-control @error('trainer_name') is-invalid @enderror" id="trainer_name" type="text" name="trainer_name" value="{{ $admintrainer->trainer_name }}">
                                        @error('trainer_name')
                                    ...
                                    <div class="form-group banner-image-field">
                                        ...
                                        <img id="image_src" src="{{ url('/backend/images/trainers_images') }}/{{ $admintrainer->image }}" alt="Trainer image">
                                    </div>
                                    ...
                                    <div class="form-group banner-image-field">
                                        <label>Email</label>
                                        <input class="form-control @error('email') is-invalid @enderror" id="email" name="email" value="{{ $admintrainer->email }}">
                                        @error('email')
                                            <div class="formAlertError alert alert-danger">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="form-group banner-image-field">
                                        <label>Phone</label>
                                        <input class="form-control @error('phone') is-invalid @enderror" id="price" name="phone" value="{{ $admintrainer->phone }}">
                                        @error('phone')
                                            <div class="formAlertError alert alert-danger">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control  @error('bio') is-invalid @enderror" id="description" name="bio">{{ $admintrainer->bio }}</textarea>
                                    </div>

                                    <div class="form-group banner-image-field">
                                        <label>Password</label>
                                        <input class="@error('password') is-invalid @enderror form-control" id="password" type="password" name="password" value="{{ old('password') }}">
                                        @error('password')
                                            <div class="formAlertError alert alert-danger">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="status" id="status" class="form-control">
                                            <option value="1" 
                                            {{ $admintrainer->status == 1 ? 'selected' : '' }}>Publish</option>
                                            <option value="0" {{ $admintrainer->status == 0 ? 'selected' : '' }}>Draft</option>
                                        </select>
                                    </div>
    ...
    jQuery(document).ready(function($){
        ...
        $('.upload-result').on('click', function(ev) {
            ...
            $uploadCrop.croppie('result', {
                ...
                $.ajax({
                    url: "/admin/trainer/image-crop-class",
                    type: "POST",
                    data: {
                        "image": resp
                    },
