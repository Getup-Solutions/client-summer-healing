...
@section('title', 'Admin | Edit Training')
@section('content')
...
<div id="page-wrapper">
    <div class="container-fluid">
            <div class="row">
                <div class="col-lg-12">
                    <div class="row">
                        <h1 class="page-header">Edit {{ $admintraining->title }} <span></h1>
                    </div>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            ...
            @if($permissiondata->contains('trainings.edit') || $userdata->id == 1)
            <div class="row">
                <div class="col-lg-12 col-md-12">
                    <div class="panel panel-default">
                        <div class="panel-body">
                            <div class="row">
                                <form role="form" style="padding:1rem 2rem;" method="post" action="{{ route('admin.dashboard.trainings.update', $admintraining->id) }}" enctype="multipart/form-data">
                                    @csrf
                                    @method('PUT')
                                    <div class="form-group">
                                        <label>Title</label>
                                        <input class="form-control @error('title') is-invalid @enderror" id="title" type="text" name="title" value="{{ $admintraining->title }}">
                                        @error('title')
                                    ...
                                    <div class="form-group banner-image-field">
                                        ...
                                        <img id="image_src" src="{{ url('/backend/images/trainings_images') }}/{{ $admintraining->image }}" alt="Training image">
                                    ...
                                    <div class="form-group banner-image-field">
                                        <label>Price</label>
                                        <input class="form-control type="text" @error('price') is-invalid @enderror" id="price" name="price" value="{{ $admintraining->price }}">
                                        @error('price')
                                            <div class="formAlertError alert alert-danger">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control  @error('description') is-invalid @enderror" id="description" name="description">{{ $admintraining->description }}</textarea>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="status" id="status" class="form-control">
                                            <option value="1" 
                                            {{ $admintraining->status == 1 ? 'selected' : '' }}>Publish</option>
                                            <option value="0" {{ $admintraining->status == 0 ? 'selected' : '' }}>Draft</option>
                                        </select>
                                    </div>
