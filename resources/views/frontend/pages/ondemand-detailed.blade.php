    ...
    <div class="container-fluid">
        <div class="row text-center text-xl-start">
            <div class="col-xl-11 mx-auto">
                <h2 class="display-4 text-center text-uppercase font-weight-700 mb-lg-4">{{ $ondemand->title }}</h2>
                @if($ondemand->excerpt)
                <div class="text-center">{!! $ondemand->excerpt !!}</div>
                @endif
            </div>
        </div>
    </div>
            ...
            <div class="container-fluid position-relative clearfix column-wrpr">
                ...
                <div class="col-left mb-4 mb-xl-0">
                    ...
                    <div class="card w-100" data-sal="slide-up" style="--sal-duration: 1s">
                        ...
                        @php 
                            $admin_trainers = $ondemand->adminondemandtrainers;
                        @endphp
                        <ul class="list-unstyled list-trainers mt-3">
                            @foreach($admin_trainers as $trainer)
                            <li class="d-flex align-items-center justify-content-start">
                                <img src="assets/images/loading.gif" data-src="{{ url("/backend/images/trainers_images") }}/{{$trainer->image}}" class="d-block w-100 lazy trainer-photo" alt="...">
                                <div class="info ms-3">
                        ...
                        <div class="card card-price" data-sal="slide-up" style="--sal-duration: 1s">
                            <div class="d-flex align-items-center justify-content-between w-100">
                                <h5 class="m-0 text-white w-100 text-center"><span
                                        class="text-muted me-3 font-weight-500 h2"><s>${{ $ondemand->price + 100 }}</s></span><span
                                        class="display-3 font-weight-700">${{ $ondemand->price }}</span></h5>
                            </div>
                            @if(session('cart'))
                                @foreach(session('cart') as $id => $details)
                                    @if($details['title'] === $ondemand->title)
                                    <a href="#" class="btn btn-primary w-100 addedcarts">Added to cart</a>
                                    @else
                                    <a href="{{ route('add.to.cart', $ondemand->id) }}?param=ondemand" class="btn btn-primary w-100">Buy now</a>
                                    @endif
                                @endforeach
                            ...
                            @else
                                ...
                                @if(auth()->check())
                                    @if($ondemandByUser->contains($ondemand->id))

                                    @else
                                         <a href="{{ route('add.to.cart', $ondemand->id) }}?param=ondemand" class="btn btn-primary w-100">Buy now</a>
                                    @endif
                                @else
                                    <a href="{{ route('add.to.cart', [$ondemand->id, $ondemand->title]) }}?param=ondemand" class="btn btn-primary w-100">Buy now</a>  
                                @endif
            ...
            <div class="row">
                <div class="col-xl-11 mx-auto">
                    ...
                    <div class="row row-equl pt-sm-5 pt-4 pb-0">
                        ...
                        @php
                            $ondemandAll = App\Models\Adminondemand::where('id', '!=', $ondemand->id)->get();
                            //dd($ondemand)
                        @endphp
                        @if($ondemandAll)
                        @foreach($ondemandAll as $ondemandOther)
                        <!-- ITEM START -->
                        <div class="col-xl-4 col-lg-6 mb-5" data-sal="slide-up" style="--sal-duration: 1s">
                            <div class="card text-center mx-2">
                                ...
                                <div class="content">
                                    <div class="middle">
                                        <a href="yoga-course-single.html"><h4 class="h4 font-weight-700 text-uppercase"> {{ $ondemandOther->title }}</h4></a>
                                        <h5 class="display-5 m-0 text-white"><span
                                                class="text-muted me-3 font-weight-500"><s>${{ $ondemandOther->price + 100 }}</s></span><span
                                                class="font-weight-700">{{ $ondemandOther->price }}</span></h5>
                                    </div>
                                    <div class="bottom w-100 d-sm-flex align-items-end justify-content-center">
                                        <a href="{{ route('page.yogaondemand.detail', $ondemandOther->slug) }}" class="btn me-3 w-100">More Info</a>
                                        <a href="{{ route('add.to.cart', $ondemandOther->id) }}" class="btn btn-primary w-100 mt-3 mt-sm-0">Buy
                                            Now</a>
                                    </div>
                                </div>
                            </div>
